...
-   **Sandbox**: Uses **E2B** (default) or Daytona.
    -   **Tools**: Injected via **MCP (Model Context Protocol)** servers running inside the sandbox with ports exposed from the sandbox out to the agent.
    -   **Apps**: Inside sandbox, exposed via ports (e.g., CodeServer/VSCode, Excalidraw, Graphite).
-   **Modules**: The agent stream supports various "Modules" (Deep Research, Data Scientist, Quant, Slides, Dev etc.) which are specialized agentic workflows.
