...
- Sandbox API Tools (Shell, File System)
- MCP Tools (via Tool Server)
- Python execution
...
class ToolTester:
    """Comprehensive tool tester using sandbox REST API and MCP Tool Server."""
    ...
    async def get_mcp_url(self) -> str:
        """Get the MCP server URL for the sandbox."""
        if self.mcp_url:
            return self.mcp_url
        ...
        if r.status_code == 200:
            self.mcp_url = r.json().get('data', {}).get('mcp_url')
            return self.mcp_url
        return None

    async def list_mcp_tools(self, mcp_url: str) -> list[str]:
        """List all available MCP tools."""
        try:
            r = await self.client.get(f"{mcp_url}/tools")
            if r.status_code == 200:
                tools = r.json()
    ...
    async def call_mcp_tool(self, mcp_url: str, tool_name: str, arguments: dict) -> tuple[bool, Any]:
        ...
        try:
            ...
            # Add timeout as some tools (like diagrams) might take longer
            r = await self.client.post(f"{mcp_url}/tools/call", json=payload, timeout=30.0)
            if r.status_code == 200:
                result = r.json()
                return True, result
            else:
    ...
    async def test_file_find(self) -> bool:
        ...
        # Use ls -R which is safer than 'find' on minimal images
        success, output = await self.run_cmd("ls -R /tmp")
...
'''
    ...
    async def test_mcp_connection(self) -> bool:
        ...
        mcp_url = await self.get_mcp_url()
        if not mcp_url:
            self.log("Could not get MCP URL", False)
            return False
            
        # Initialize MCP (Register tools manually since we used raw sandbox create)
        try:
            # 1. Set credentials
            await self.client.post(f"{mcp_url}/credential", json={
                "user_api_key": "test_key",
                "session_id": "test_session_tools"
            })
            # 2. Set tool server URL (Triggers registration)
            await self.client.post(f"{mcp_url}/tool-server-url", json={
                "tool_server_url": "http://localhost:6060"
            })
            # Give it a moment to register tools
            await asyncio.sleep(1.0)
        except Exception as e:
            self.log(f"MCP Initialization warning: {e}", None)

        self.log(f"Connected to MCP endpoint: {mcp_url}", True)
        
        tools = await self.list_mcp_tools(mcp_url)
        self.log(f"Discovered {len(tools)} MCP tools", True)
        
        # Verify critical tools exist (from list_mcp_tools.py)
        critical_tools = [
            "excalidraw_init", "design_init", "file_read", 
            "shell_run_command", "document_init"
        ]
        
        found_all = True
        for t in critical_tools:
            if t not in tools:
                self.log(f"Missing critical tool: {t}", False)
                found_all = False
        
        return found_all

    async def test_mcp_file_write_read(self) -> bool:
        """Test: MCP file_write and file_read tools."""
        mcp_url = await self.get_mcp_url()
        content = "MCP_TOOL_CONTENT_TEST"
        path = "/workspace/mcp_test_file.txt" # Using /workspace as it's safe
        
        # 1. Write
        success, result = await self.call_mcp_tool(
            mcp_url, 
            "file_write", 
            {"path": path, "content": content}
        )
        if not success:
            self.log(f"MCP file_write failed: {result}", False)
            return False
        ...
        if not success:
            self.log(f"MCP file_read failed: {result}", False)
            return False
            
        # Parse result (MCP 1.0 vs FastMCP return format issues)
        try:
            ...
            if isinstance(result, dict):
                ...
                if content_list and isinstance(content_list, list):
                    content_val = content_list[0].get("text", "")
            
            return content in content_val
        except Exception as e:
            self.log(f"Error parsing MCP result: {e}", False)
            return False

    async def test_mcp_excalidraw_init(self) -> bool:
        """Test: MCP excalidraw_init tool."""
        mcp_url = await self.get_mcp_url()
        success, result = await self.call_mcp_tool(
            mcp_url, 
            "excalidraw_init", 
            {"session_name": "test_session_automated"}
        )
        ...
        if success and isinstance(result, dict):
            ...
            if content_list:
                ...
                return "Successfully initialized" in text and "Viewer URL" in text
        return False

    async def test_mcp_design_init(self) -> bool:
        """Test: MCP design_init tool."""
        mcp_url = await self.get_mcp_url()
        success, result = await self.call_mcp_tool(
            mcp_url, 
            "design_init", 
            {"session_name": "test_design_automated"}
        )
        if success and isinstance(result, dict):
            content_list = result.get("content", [])
            if content_list:
                text = content_list[0].get("text", "")
                return "Design session" in text and "initialized" in text
        return False
    ...
    async def run_all_tests(self):
        ...
        test_groups = {
            ...
            "🚀 MCP TOOLS (Actual Tool Server)": [
                ("mcp_connection", self.test_mcp_connection),
                ("mcp_file_write_read", self.test_mcp_file_write_read),
                ("mcp_excalidraw_init", self.test_mcp_excalidraw_init),
                ("mcp_design_init", self.test_mcp_design_init),
            ]
        }
