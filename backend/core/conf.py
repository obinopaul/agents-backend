...
class Settings(BaseSettings):
    ...
    WS_RATE_LIMIT_ENABLED: bool = True  # Enable/disable rate limiting
    WS_RATE_LIMIT_REDIS_PREFIX: str = 'agents_backend:ws:limiter'  # Redis key prefix

    # CORS
    CORS_ALLOWED_ORIGINS: list[str] = [  # 末尾不带斜杠
        'http://127.0.0.1:8000',
        'http://localhost:8000',
        'http://localhost:1420',  # Frontend (Vite/Tauri)
        'http://127.0.0.1:1420',
    ...
    # Google Drive Connector OAuth (separate from login OAuth)
    # This should be a frontend URL that captures the OAuth callback and sends it to the backend
    ...
    # E2B Configuration
    E2B_API_KEY: str = ''
    E2B_TEMPLATE_ID: str = 'd26u5o4fokcen2fruvmz'
    ...
    SANDBOX_CODEX_SSE_PORT: int = 1324  # Codex native SSE server port
    SANDBOX_LATEX_EDITOR_PORT: int = 9001  # LaTeX AI Editor port (Documents mode)
    SANDBOX_DESIGN_MCP_PORT: int = 6002  # Design MCP Server port (Draw.io mode)
    SANDBOX_EXCALIDRAW_PORT: int = 6003  # Excalidraw server port
    SANDBOX_GRAPHITI_MCP_PORT: int = 8500  # Graphiti MCP HTTP server port (Knowledge Graph)
    SANDBOX_GRAPHITI_WEB_UI_PORT: int = 3500  # FalkorDB Web UI port (optional, for debugging)
    
    # Tool Server Configuration (runs in Docker stack, provides web search, image gen, etc.)
    # This is DIFFERENT from MCP server (which runs inside sandbox)
    TOOL_SERVER_PORT: int = 1237  # Local Tool Server port
    PUBLIC_TOOL_SERVER_URL: str = ''  # ngrok URL or production URL (e.g., https://xxx.ngrok.io)
    
    # ngrok Configuration (for Tool Server tunnel to E2B sandboxes)
    NGROK_AUTHTOKEN: str = ''  # ngrok auth token
    NGROK_REGION: str = 'us'  # ngrok region (us, eu, ap, au)

    # --------------------------------------------------------------------------
    # [Google Cloud Storage Configuration]
    # Used by tool server for video/image generation output, slide assets, etc.
    # --------------------------------------------------------------------------
    GCS_PROJECT_ID: str = ''  # Google Cloud project ID
    GCS_BUCKET_NAME: str = ''  # Primary bucket for tool server storage
    GCS_OUTPUT_BUCKET: str = ''  # Output bucket for video/image generation
    ...
    # Storage Backend: 's3' for S3-compatible storage (production default)
    # Set to 'local' in .env only for local development without S3
