...
class CRUDUser(CRUDPlus[User]):
    ...
    async def add_by_registration(
        ...
        *,
        ...
        """
        Add user via public registration (email/password signup).

        Uses bcrypt for password hashing (same as create_test_user.py).
        Auto-assigns first available role and sets user as active.

        :param db: Database session
        :param username: Unique username (typically derived from email)
        ...
        """
        # Hash password using bcrypt (consistent with create_test_user.py)
        salt = bcrypt.gensalt()
        hashed_password = get_hash_password(password, salt)

        # Create user with default settings for public registration
        new_user = self.model(
            username=username,
            nickname=nickname,
            ...
            is_staff=True,  # Can use the platform
            is_multi_login=True,  # Allow multiple device login
        )
        db.add(new_user)
        await db.flush()

        # Assign default role (first available role)
        role_stmt = select(Role)
        result = await db.execute(role_stmt)
