...
-------------------------
Database-backed endpoints (/db/*) MUST be defined BEFORE sandbox endpoints
(/{sandbox_id}/*) to ensure FastAPI matches the static "/db" prefix before
trying to match it as a dynamic sandbox_id path parameter.

Database Endpoints (thread_id based):
- GET  /db/presentations - List presentations from database
- GET  /db/slide - Get slide content from database
- POST /db/slide - Write slide to database
- GET  /db/download - Export to PDF from database
- GET  /db/download/stream - SSE streaming PDF export

Sandbox Endpoints (sandbox_id based, legacy):
- GET  /{sandbox_id}/presentations - List presentations in sandbox filesystem
- GET  /{sandbox_id}/presentations/{name} - List slides in presentation
- GET  /{sandbox_id}/slides/{name}/{num} - Get slide HTML content
- POST /{sandbox_id}/slides/export - Export presentation to PDF
- GET  /{sandbox_id}/slides/download/{name} - Download as ZIP
"""
...
frontend_router = APIRouter(tags=["Frontend Slides"]) # Router for frontend aliases
...
# IMPORTANT: These routes MUST be defined BEFORE the /{sandbox_id}/* routes
# to ensure FastAPI matches the static "/db/" prefix before the dynamic
# path parameter pattern "/{sandbox_id}/".
# ============================================================================
...
@router.get(
    "/db/download/stream",
    summary="Stream PDF download progress",
    description="Export slides from database to PDF with SSE progress updates.",
    dependencies=[DependsJwtAuth],
)
async def download_db_slides_pdf_stream(
    db: CurrentSession,
    thread_id: str = Query(..., description="Thread ID"),
    presentation_name: Optional[str] = Query(None, description="Specific presentation"),
):
    """
    Export slides to PDF with Server-Sent Events progress updates.
    
    Emits:
    - progress: {"current": n, "total": m, "percent": p, "message": "..."}
    - complete: {"pdf_bytes": base64_encoded, "total_pages": n}
    - error: {"message": "..."}
    """
    from backend.src.services.slides.pdf_service import convert_slides_to_pdf_with_progress
    ...
    async def generate():
        try:
            ...
            if not slides:
                yield f"event: error\ndata: {json.dumps({'message': 'No slides found'})}\n\n"
                return
            
            # Convert with progress
            async for update in convert_slides_to_pdf_with_progress(slides):
                if update["type"] == "progress":
                    yield f"event: progress\ndata: {json.dumps(update)}\n\n"
                elif update["type"] == "complete":
                    # Base64 encode the PDF for SSE transport
                    pdf_b64 = base64.b64encode(update["pdf_bytes"]).decode()
                    yield f"event: complete\ndata: {json.dumps({'pdf_base64': pdf_b64, 'total_pages': update['total_pages']})}\n\n"
...
@frontend_router.get("/download/stream", dependencies=[DependsJwtAuth])
async def download_slides_stream_alias(
    db: CurrentSession,
    thread_id: str = Query(..., description="Thread ID"),
    presentation_name: Optional[str] = Query(None, description="Specific presentation"),
):
    """Alias for /db/download/stream matching frontend expectation."""
    # Reuse the implementation
    return await download_db_slides_pdf_stream(db, thread_id, presentation_name)



# ============================================================================
# Sandbox-Based Endpoints (sandbox_id based)
# These endpoints read slides directly from the sandbox filesystem.
# They require an active sandbox with the specified sandbox_id.
# ============================================================================
