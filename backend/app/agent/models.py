...
class ToolCall(BaseModel):
    ...
    def from_langchain(cls, lc_tool_call: Dict[str, Any]) -> "ToolCall":
        ...
        return cls(
            tool_call_id=lc_tool_call.get("id") or str(uuid4()),
            tool_name=lc_tool_call.get("name") or "",
            tool_input=lc_tool_call.get("args") or {},
...
class AgentRequest(BaseModel):
    ...
    enable_background_investigation: bool = Field(default=False)
    enable_web_search: bool = Field(default=True)
    locale: str = Field(default="en-US")
    enable_added_tools: bool = Field(default=False)
