...
class LLMSummarizingContextManager(ContextManager):
    ...
    async def apply_truncation(
        ...
        if has_thinking_blocks:
            return await self._apply_truncation_with_thinking_blocks(message_lists)
        else:
            return await self._apply_truncation_without_thinking_blocks(message_lists)

    async def _apply_truncation_with_thinking_blocks(
        self, message_lists: list[list[GeneralContentBlock]]
    ) -> list[list[GeneralContentBlock]]:
        ...
        summary = await self._generate_summary(events_to_summarize)
    ...
    async def _apply_truncation_without_thinking_blocks(
        self, message_lists: list[list[GeneralContentBlock]]
    ) -> list[list[GeneralContentBlock]]:
        ...
        summary = await self._generate_summary(forgotten_events, summary_content)
    ...
    async def _generate_summary(self, forgotten_events: list[list[GeneralContentBlock]], previous_summary_content: str = "No events summarized") -> str:
