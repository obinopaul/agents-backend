...
async def get_settings_store(request: Request) -> SettingsStore:
    settings_store: SettingsStore | None = getattr(
        request.state, "settings_store", None
    )
    if settings_store:
        return settings_store
    settings_store = await FileSettingsStore.get_instance(shared.config, user_id=None)
