        ...
        *,
        ...
        match social:
            case UserSocialType.github:
                ...
                # Falls back to login username if name is not set
                nickname = user.get('name') or user.get('login') or username
            case UserSocialType.google:
                sid = user.get('id')
                username = user.get('name')
                # Use full name (e.g. "Paul Okafor") so first_name/last_name
                # are properly derived; fall back to given_name if not available
                nickname = user.get('name') or user.get('given_name') or username
                avatar = user.get('picture')
            case UserSocialType.linux_do:
                sid = user.get('id')
                nickname = user.get('name') or username
            case _:
