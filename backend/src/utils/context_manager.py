...
def validate_message_content(messages: List[BaseMessage], max_content_length: int = 100000) -> List[BaseMessage]:
    ...
    5. AIMessage tool_calls have valid string IDs
    6. ToolMessage tool_call_id is a valid string
    ...
    for i, msg in enumerate(messages):
        try:
            ...
            # ===== VALIDATE TOOL_CALL IDS (fixes 400 BadRequestError with invalid_type) =====
            
            # Validate AIMessage tool_calls
            if isinstance(msg, AIMessage) and hasattr(msg, 'tool_calls') and msg.tool_calls:
                for j, tool_call in enumerate(msg.tool_calls):
                    # Check if it's a dict (legacy format) or ToolCall object
                    if isinstance(tool_call, dict):
                        tc_id = tool_call.get('id')
                        if tc_id is None or not isinstance(tc_id, str):
                            logger.warning(f"Message {i} tool_calls[{j}].id is invalid ({type(tc_id).__name__}), generating new ID")
                            import uuid
            ...
            # Also check additional_kwargs for tool_calls (some LLM providers use this)
                ...
                if 'tool_calls' in msg.additional_kwargs:
                    for j, tool_call in enumerate(msg.additional_kwargs['tool_calls']):
                        if isinstance(tool_call, dict):
                            tc_id = tool_call.get('id')
                            if tc_id is None or not isinstance(tc_id, str):
                                logger.warning(f"Message {i} additional_kwargs.tool_calls[{j}].id is invalid, generating new ID")
                                import uuid
                                tool_call['id'] = f"call_{uuid.uuid4().hex[:24]}"
            
            # Validate ToolMessage tool_call_id
            if isinstance(msg, ToolMessage):
                if not hasattr(msg, 'tool_call_id') or msg.tool_call_id is None or not isinstance(msg.tool_call_id, str):
                    logger.warning(f"Message {i} ToolMessage has invalid tool_call_id ({getattr(msg, 'tool_call_id', 'missing')}), generating new ID")
                    import uuid
