...
// Session state (initialized to default session)
let currentSession: {
    id: string
    xml: string
    version: number
...
server.registerTool(
    ...
    async () => {
        try {
            ...
            // Initialize state and set as current for viewer
...
async function main() {
    ...
    // Start HTTP server at initialization so port 6002 is available immediately.
    // This enables HTTP clients (backend design tools) to connect before any
    // MCP tool calls are made. The HTTP server handles:
    // - Browser viewer at /?mcp=SESSION_ID
    // - State API at /api/state
    // - History API at /api/history
    //
    // This fixes the "connection refused" issue where backend tools try to
    // connect via HTTP, but the server was only started via stdio start_session.
    try {
        const port = await startHttpServer(config.port)
        log.info(`HTTP server running on port ${port}`)
    } catch (error) {
        // Log error but continue - MCP stdio transport can still function
        // This allows graceful degradation if port is unavailable
        const message = error instanceof Error ? error.message : String(error)
        log.error(`Failed to start HTTP server: ${message}`)
        log.warn("Continuing without HTTP server - stdio transport only")
    }

    // Initialize default session at startup
    log.info(`Initializing default session: ${DEFAULT_SESSION_ID}`)
    setState(DEFAULT_SESSION_ID, DEFAULT_DIAGRAM_XML)
