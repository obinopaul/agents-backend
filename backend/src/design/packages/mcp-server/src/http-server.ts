...
export const DEFAULT_DIAGRAM_XML = `<mxfile host="app.diagrams.net"><diagram id="blank" name="Page-1"><mxGraphModel><root><mxCell id="0"/><mxCell id="1" parent="0"/></root></mxGraphModel></diagram></mxfile>`
...
// Current session tracking - the session the agent is currently working on
// Viewer will auto-follow this session when visiting base URL (no ?mcp= param)
export const DEFAULT_SESSION_ID = "mcp-default"
...
export function setCurrentSessionId(sessionId: string): void {
    ...
    log.info(`Current session set to: ${sessionId}`)
}
...
function handleRequest(
    ...
    if (url.pathname === "/" || url.pathname === "/index.html") {
        // If no session ID in URL, use the current session (agent's active session)
        let sessionId = url.searchParams.get("mcp") || ""
        if (!sessionId) {
    ...
    } else if (url.pathname === "/api/current-session") {
        // Returns the current session ID - viewer can poll this to auto-follow
        res.writeHead(200, { "Content-Type": "application/json" })
        res.end(JSON.stringify({ 
            sessionId: currentSessionId,
            isDefault: currentSessionId === DEFAULT_SESSION_ID
        }))
...
function handleStateApi(
    ...
    if (req.method === "GET") {
        ...
        return  // CRITICAL: Prevent fall-through to 405 response
        ...
        req.on("end", () => {
            try {
                ...
                if (!sessionId || !xml) {
                    res.writeHead(400, { "Content-Type": "application/json" })
                    res.end(JSON.stringify({ error: "sessionId and xml required" }))
                    return
                }

                // Update state and history
                const newVersion = setState(sessionId, xml)
                addHistory(sessionId, xml, svg)

                // AUTO-FOLLOW: If an external tool writes to a session, switch viewer to it
                setCurrentSessionId(sessionId)

                log.info(`Updated session ${sessionId} to version ${newVersion}`)
                ...
                res.writeHead(400, { "Content-Type": "application/json" })
                res.end(JSON.stringify({ error: "Invalid Request", details: message }))
            }
        })
        return
    }
...
<body>
    ...
    <script>
        ...
        const hasExplicitSession = !!sessionId;
        
        // Auto-follow logic: If no explicit session in URL, track current session
        async function checkCurrentSession() {
            if (hasExplicitSession) return;
            try {
                const r = await fetch('/api/current-session');
                if (r.ok) {
                    const data = await r.json();
                    if (data.sessionId && data.sessionId !== sessionId) {
                        // Session changed! Reload to switch
                        console.log('Session switch detected:', data.sessionId);
                        window.location.reload();
                    }
                }
            } catch (e) { console.error('Auto-follow check failed', e); }
        }

        // Poll for session changes if we're on the default view
        if (!hasExplicitSession) {
            setInterval(checkCurrentSession, 2000);
        }
        ...
        window.addEventListener('message', (e) => {
            ...
            try {
                ...
                if (msg.event === 'init') {
                    ...
                    if (pendingXml) { loadDiagram(pendingXml); pendingXml = null; } // STARTUP LOAD FIX
