...
class SandboxConfig(BaseModel):
    ...
    def __init__(self, **data):
        ...
        # Note: Handle empty string from pydantic settings as falsy
        e2b_key = getattr(settings, "E2B_API_KEY", None)
        if not e2b_key:  # Empty string or None
            e2b_key = os.getenv("E2B_API_KEY")
        data.setdefault("e2b_api_key", e2b_key if e2b_key else None)
        data.setdefault("e2b_template_id", getattr(settings, "E2B_TEMPLATE_ID", "d26u5o4fokcen2fruvmz") or os.getenv("E2B_TEMPLATE_ID", "d26u5o4fokcen2fruvmz"))
        ...
        data.setdefault("anthropic_api_key", getattr(settings, "ANTHROPIC_API_KEY", None) or os.getenv("ANTHROPIC_API_KEY"))
        data.setdefault("google_api_key", getattr(settings, "GOOGLE_API_KEY", None) or os.getenv("GOOGLE_API_KEY"))
        data.setdefault("voyage_api_key", getattr(settings, "VOYAGE_API_KEY", None) or os.getenv("VOYAGE_API_KEY"))
    ...
    # API keys to pass to sandbox (for Graphiti and other services)
    openai_api_key: Optional[str] = Field(
        default=None, description="OpenAI API key for services like Graphiti"
    )
    anthropic_api_key: Optional[str] = Field(
        default=None, description="Anthropic API key for services like Graphiti"
    )
    google_api_key: Optional[str] = Field(
        default=None, description="Google API key for services like Graphiti"
    )
    voyage_api_key: Optional[str] = Field(
        default=None, description="Voyage API key for services like Graphiti"
    )
