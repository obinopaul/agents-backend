...
class SandboxTable:
    ...
    async def update_session_sandbox(self, session_id: str, sandbox_id: str, user_id: int = None) -> bool:
        """Link a sandbox to a session by updating SessionMetrics.sandbox_id.

        Args:
            session_id: The session ID to update
            sandbox_id: The sandbox ID to link
            user_id: The user ID for new session records (required for FK constraint)
        ...
        try:
            ...
            async with get_db() as db:
                ...
                if session_metrics:
                    ...
                    await db.flush()
                    logger.info(f"Updated existing session {session_id} with sandbox {sandbox_id}")
                    return True
                else:
                    # Create new SessionMetrics record with sandbox_id
                    if user_id is None:
                        logger.warning(f"Cannot create SessionMetrics without user_id for session {session_id}")
                        return False
                    
                    new_session = SessionMetrics(
                        session_id=session_id,
                        sandbox_id=sandbox_id,
                        user_id=user_id,  # Real user ID from JWT token
                    )
                    db.add(new_session)
                    await db.flush()
                    logger.info(f"Created new session {session_id} with sandbox {sandbox_id} for user {user_id}")
                    return True
        except Exception as e:
