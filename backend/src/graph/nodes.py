...
def background_investigation_node(state: State, config: RunnableConfig):
    ...
    if not state.get("enable_background_investigation", False):
...
async def _execute_agent_step(
    ...
    # =========================================================================
    # HITL Guard: Prevent infinite feedback loops
    # =========================================================================
    # If the agent is requesting feedback AGAIN after already receiving a human
    # response, suppress the repeat request. This prevents the infinite loop
    # where the agent keeps calling request_human_input on every iteration.
    if needs_feedback:
        all_messages = list(state.get("messages", [])) + list(agent_messages)
        prior_human_feedback_count = sum(
            1 for msg in all_messages
            if isinstance(msg, HumanMessage) and getattr(msg, "name", None) in ("human_feedback", "human_decision")
        )
        prior_hitl_request_count = sum(
            1 for msg in all_messages
            if hasattr(msg, 'tool_calls') and any(
                tc.get('name') == "request_human_input" for tc in (msg.tool_calls or [])
            )
        )
        
        if prior_hitl_request_count > 1:
            logger.warning(
                f"[HITL Guard] Suppressing repeat HITL request. "
                f"Prior HITL requests: {prior_hitl_request_count}, "
                f"Prior human responses: {prior_human_feedback_count}. "
                f"Forcing __end__ to prevent infinite loop."
            )
            needs_feedback = False
            hitl_questions = None
        elif prior_human_feedback_count >= 1 and prior_hitl_request_count >= 1:
            logger.warning(
                f"[HITL Guard] Agent requesting feedback again after user already responded. "
                f"Suppressing to prevent loop. Responses: {prior_human_feedback_count}"
            )
            needs_feedback = False
            hitl_questions = None
...
async def _setup_and_execute_agent_step(
    ...
    *,
    ...
    # Uses retry logic because the sandbox MCP server may have transient
    # connectivity issues (e.g., cold start timing, network glitches).
    if mcp_servers:
        ...
        for attempt in range(1, max_retries + 1):
            try:
                logger.info(
                    f"[MCP_TOOLS] Loading MCP tools from {len(mcp_servers)} server(s): "
                    f"{list(mcp_servers.keys())} (attempt {attempt}/{max_retries})"
                )

                # Optional: pre-flight health check for sandbox MCP
                if configurable.mcp_url and attempt == 1:
                    try:
                        import httpx
                        async with httpx.AsyncClient() as _http:
                            health_resp = await _http.get(
                                f"{configurable.mcp_url}/health", timeout=10.0
                            )
                            if health_resp.status_code == 200:
                                logger.info("[MCP_TOOLS] Sandbox MCP health check passed")
                            else:
                                logger.warning(
                                    f"[MCP_TOOLS] Sandbox MCP health check returned {health_resp.status_code}, "
                                    "will still attempt tool loading"
                                )
                    except Exception as health_err:
                        logger.warning(
                            f"[MCP_TOOLS] Sandbox MCP health check failed: {health_err}. "
                            "Will still attempt tool loading."
                        )
                ...
                logger.info(f"[MCP_TOOLS] get_tools() returned {len(all_tools)} tools on attempt {attempt}")
                if all_tools:
                    for t in all_tools:
                        logger.info(f"[MCP_TOOLS] Found tool: {t.name}")
                    break  # Success — exit retry loop
                else:
                    # Empty list might mean server returned no tools (transient)
                    logger.warning(
                        f"[MCP_TOOLS] get_tools() returned 0 tools on attempt {attempt}/{max_retries}"
                    )
                ...
                last_error = e
                logger.warning(
                    f"[MCP_TOOLS] Failed to load MCP tools on attempt {attempt}/{max_retries}: {e}"
                )
                if attempt < max_retries:
                    await _asyncio.sleep(retry_delay)
                else:
                    logger.error(
                        f"[MCP_TOOLS] All {max_retries} attempts to load MCP tools failed. "
                        f"Last error: {e}",
                        exc_info=True,
                    )

        # Process loaded tools (may be empty if all retries failed)
        mcp_tool_count = 0
        for tool in all_tools:
            # Logic:
            # 1. If tool is in enabled_tools (from static config), accept it
            # 2. If we have a dynamic sandbox, ACCEPT ALL tools (assumed to be from sandbox)
        ...
        logger.info(f"[MCP_TOOLS] Successfully loaded {mcp_tool_count} MCP tools (from {len(all_tools)} available)")

        if mcp_tool_count == 0 and configurable.mcp_url:
            logger.error(
                f"[MCP_TOOLS] WARNING: mcp_url is set ({configurable.mcp_url}) but "
                f"0 MCP tools were loaded! Last error: {last_error}. "
                "Agent will run without sandbox tools (file_read, shell_run_command, "
                "excalidraw_*, etc). This likely means the sandbox MCP server is "
                "not responding or returned no tools."
            )
    ...
    custom_config = {
        ...
        "enable_background_tasks": False,  # Requires store for subagents
        "enable_skills": True,
