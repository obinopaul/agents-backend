...
class PostgresCheckpointerManager:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            # Create the connection pool with robust settings
            # Mirrors patterns from FastAPI db.py (pool_recycle) and II-Agent
            self._pool = AsyncConnectionPool(
                db_url,
                min_size=pool_min_size,
                max_size=pool_max_size,
                timeout=pool_timeout,
                # Robust connection lifecycle settings (equivalent to SQLAlchemy's pool_recycle/pool_pre_ping)
                max_lifetime=1800,         # Recycle connections every 30 min (prevents stale connections)
                max_idle=600,              # Close idle connections after 10 min
                reconnect_timeout=30,      # Timeout for reconnection attempts
                ...
                check=AsyncConnectionPool.check_connection,  # Health check before checkout (like pool_pre_ping)
    ...
    async def warm_up_connection(self, max_retries: int = 3, base_delay: float = 1.0) -> bool:
        """
        Warm up the database connection before long-running operations.
        
        This method ensures the PostgreSQL connection is alive and ready,
        preventing "connection unexpectedly closed" errors during astream_events.
        Uses exponential backoff retry pattern (similar to II-Agent with_retry).
        ...
        Returns:
            True if connection is ready, False if all retries failed
        """
        if not self._initialized or self._pool is None:
            logger.warning("Checkpointer not initialized, attempting initialization")
            try:
                await self.initialize()
            except Exception as e:
                logger.error(f"Failed to initialize checkpointer during warm-up: {e}")
                return False
        ...
        for attempt in range(max_retries + 1):
            try:
                async with self._pool.connection() as conn:
                    ...
                    logger.debug(f"Database connection warm-up successful (attempt {attempt + 1})")
                    return True
            except Exception as e:
                last_exception = e
                if attempt < max_retries:
                    ...
                    logger.warning(
                        f"Database warm-up failed (attempt {attempt + 1}/{max_retries + 1}), "
                        f"retrying in {delay}s: {e}"
                    )
                    await asyncio.sleep(delay)
                else:
                    logger.error(
                        f"Database warm-up failed after {max_retries + 1} attempts: {last_exception}"
                    )
        
        # All retries exhausted - try to reinitialize the pool
        logger.warning("All warm-up retries failed, reinitializing connection pool")
        try:
            await self.shutdown()
            await self.initialize()
            ...
            async with self._pool.connection() as conn:
                await conn.execute("SELECT 1")
                logger.info("Database connection restored after pool reinitialization")
                return True
        except Exception as e:
