...
class MCPTransportTester:
    ...
    async def test_get_episodes(self) -> bool:
        ...
        try:
            result = await self.session.call_tool(
                'graphiti_get_episodes', {'group_ids': [self.test_group_id], 'limit': 10}
            )
    ...
    async def test_clear_graph(self) -> bool:
        ...
        try:
            result = await self.session.call_tool('graphiti_clear_graph', {'group_id': self.test_group_id})
