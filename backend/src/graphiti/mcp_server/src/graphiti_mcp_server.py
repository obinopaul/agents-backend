...
Key capabilities:
1. Add episodes (text, messages, or JSON) to the knowledge graph with the graphiti_add_memory tool
2. Search for nodes (entities) in the graph using natural language queries with graphiti_search_nodes
3. Find relevant facts (relationships between entities) with graphiti_search_facts
4. Retrieve specific entity edges or episodes by UUID
5. Manage the knowledge graph with tools like graphiti_delete_episode, graphiti_delete_entity_edge, and graphiti_clear_graph
...
@mcp.tool()
async def graphiti_search_memory_facts(
    query: str,
    group_ids: list[str] | None = None,
    max_facts: int = 10,
...
@mcp.tool()
async def graphiti_delete_entity_edge(uuid: str) -> SuccessResponse | ErrorResponse:
...
@mcp.tool()
async def graphiti_delete_episode(uuid: str) -> SuccessResponse | ErrorResponse:
...
@mcp.tool()
async def graphiti_get_entity_edge(uuid: str) -> dict[str, Any] | ErrorResponse:
...
@mcp.tool()
async def graphiti_clear_graph(group_ids: list[str] | None = None) -> SuccessResponse | ErrorResponse:
...
@mcp.tool()
async def graphiti_get_status() -> StatusResponse:
...
@mcp.custom_route('/credential', methods=['POST'])
async def set_credential(request) -> JSONResponse:
    """Set credentials at runtime (e.g. OpenAI, Anthropic, Gemini API Keys)."""
    global config, graphiti_service, graphiti_client, queue_service
    ...
    # Handle both legacy payload {"user_api_key": "..."} and new rich payload {"credentials": {...}}
    credentials = data.get('credentials', {})
    
    # Support legacy single-key injection (defaults to OpenAI if not specified in config)
    user_api_key = data.get('user_api_key')
    if user_api_key and not credentials:
        # Infer provider from config if possible
        default_provider = config.llm.provider.lower()
        credentials[default_provider] = user_api_key
        
    if not credentials:
         return JSONResponse({'error': 'No credentials provided'}, status_code=400)

    updated_providers = []

    # Update OpenAI
    if credentials.get('openai'):
        if not config.llm.providers.openai:
            from config.schema import OpenAIProviderConfig
            config.llm.providers.openai = OpenAIProviderConfig()
        config.llm.providers.openai.api_key = credentials['openai']
        
        # Also update embedder
        if not config.embedder.providers.openai:
            from config.schema import OpenAIProviderConfig
            config.embedder.providers.openai = OpenAIProviderConfig()
        config.embedder.providers.openai.api_key = credentials['openai']
        updated_providers.append('openai')

    # Update Anthropic
    if credentials.get('anthropic'):
        if not config.llm.providers.anthropic:
            from config.schema import AnthropicProviderConfig
            config.llm.providers.anthropic = AnthropicProviderConfig()
        config.llm.providers.anthropic.api_key = credentials['anthropic']
        updated_providers.append('anthropic')

    # Update Gemini
    if credentials.get('gemini'):
        if not config.llm.providers.gemini:
             from config.schema import GeminiProviderConfig
             config.llm.providers.gemini = GeminiProviderConfig()
        config.llm.providers.gemini.api_key = credentials['gemini']
        
        # Also update embedder
        if not config.embedder.providers.gemini:
             from config.schema import GeminiProviderConfig
             config.embedder.providers.gemini = GeminiProviderConfig()
        config.embedder.providers.gemini.api_key = credentials['gemini']
        updated_providers.append('gemini')
        
    # Update Voyage
    if credentials.get('voyage'):
        if not config.embedder.providers.voyage:
            from config.schema import VoyageProviderConfig
            config.embedder.providers.voyage = VoyageProviderConfig()
        config.embedder.providers.voyage.api_key = credentials['voyage']
        updated_providers.append('voyage')

    logger.info(f"Updated credentials for: {', '.join(updated_providers)}")
    ...
    if graphiti_service:
        try:
            ...
            # Re-init queue service if needed
            if queue_service:
                await queue_service.initialize(graphiti_client)
                
            logger.info("Successfully re-initialized Graphiti with new credentials")
    ...
    return JSONResponse({'error': 'Graphiti service not available'}, status_code=500)
...
async def initialize_server() -> ServerConfig:
    ...
    # Try initialize, but allow fail (waiting for credentials via API)
    try:
        await graphiti_service.initialize()
        ...
        logger.warning(f"Initial Graphiti startup failed (likely waiting for credentials): {e}")
        logger.info("Server will start; use POST /credential to provide API key.")
