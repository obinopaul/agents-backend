...
class FileStorageBackend(ABC):
    ...
    async def get_upload_url(
        ...
        """
        Get a presigned URL/fields to upload a file.
        ...
        Returns:
            Dict containing 'url' and optional 'fields' for the upload,
...
class LocalFileStorage(FileStorageBackend):
    ...
    async def get_upload_url(
        ...
        """Get upload URL for local storage.

        Note: Local storage doesn't support presigned URLs.
        Returns None - use S3 storage for production file uploads.
        """
        # Local storage doesn't support presigned uploads.
        # For production, always use S3-compatible storage.
        return None
...
class S3FileStorage(FileStorageBackend):
    ...
    async def get_upload_url(
        ...
        Uses PUT-based presigned URL which is compatible with the frontend's
        fetch/PUT upload approach. The frontend sends the file as a raw binary
        body with a PUT request, not as form data with POST.
        """
        key = self._sanitize_key(storage_path)

        loop = asyncio.get_event_loop()
        try:
            # Use generate_presigned_url with put_object for PUT uploads
            # This matches the frontend's PUT request with raw binary body
            url = await loop.run_in_executor(
                None,
                lambda: self._client.generate_presigned_url(
                    "put_object",
            ...
            # Return consistent format with empty fields (no form fields needed for PUT)
...
def get_storage_backend(
    ...
    **kwargs,
    ...
    Uses settings from backend.core.conf. S3 is the default for production.

    Args:
        backend_type: "local" or "s3" (defaults to settings.FILE_STORAGE_BACKEND)
        **kwargs: Backend-specific configuration (overrides settings)
    ...
    # Use settings if not explicitly provided (default is 's3')
    if backend_type is None:
        backend_type = getattr(settings, 'FILE_STORAGE_BACKEND', 's3')

    if backend_type == "s3":
        # S3-compatible storage (AWS S3, Cloudflare R2, MinIO, etc.)
        _storage_instance = S3FileStorage(
            bucket=kwargs.get("bucket") or getattr(
                settings, 'FILE_STORAGE_S3_BUCKET', 'staged-files'
            ),
            endpoint_url=kwargs.get("endpoint_url") or getattr(
                settings, 'FILE_STORAGE_S3_ENDPOINT_URL', None
            ) or None,
            region=kwargs.get("region") or getattr(
                settings, 'FILE_STORAGE_S3_REGION', 'us-east-1'
            ),
            access_key=kwargs.get("access_key") or getattr(
                settings, 'FILE_STORAGE_S3_ACCESS_KEY', None
            ) or None,
            secret_key=kwargs.get("secret_key") or getattr(
                settings, 'FILE_STORAGE_S3_SECRET_KEY', None
            ) or None,
            public_url_base=kwargs.get("public_url_base") or getattr(
                settings, 'FILE_STORAGE_S3_PUBLIC_URL_BASE', None
            ) or None,
        )

    elif backend_type == "local":
        # Local storage (only for development without S3)
        base_path = kwargs.get("base_path") or getattr(
            settings, 'FILE_STORAGE_LOCAL_PATH', './uploads/staged-files'
        )
        base_url = kwargs.get("base_url") or getattr(
            settings, 'FILE_STORAGE_LOCAL_BASE_URL', None
        ) or None
