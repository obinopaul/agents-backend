...
"""
FastParse Core Parser - Comprehensive file parsing and text extraction.

Adapted from SUNA project's fast_parse/parser.py.
Supports PDF, Word, Excel, PowerPoint, text files, and images.

Usage:
    from backend.src.services.file_processing.fast_parse import parse
    
    result = parse(file_bytes, "document.pdf")
    if result.success:
...
class ParseResult:
    ...
    Attributes:
        ...
        content: Extracted text content (empty for images/binary)
    ...
    def char_count(self) -> int:
        """Count of characters in content."""
        return len(self.content) if self.content else 0
    
    @property
    def line_count(self) -> int:
        """Count of lines in content."""
...
class FastParse:
    """
    Fast file parser with support for multiple document formats.
    
    Example:
        parser = FastParse()
        result = parser.parse(file_bytes, "document.pdf")
    """
    ...
    def _build_extension_map(self) -> Dict[str, FileType]:
        ...
        for ext in self._config.audio_extensions:
            ext_map[ext.lower()] = FileType.AUDIO
        for ext in self._config.video_extensions:
    ...
    def parse(
        ...
        Args:
            content: File content (bytes, file-like object, or string)
            filename: Original filename
        ...
        if file_size > self._config.max_file_size_bytes:
            return ParseResult(
                ...
                file_size=file_size,
                error=f"File exceeds maximum size limit of {format_file_size(self._config.max_file_size_bytes)}",
            )
        ...
        try:
            if file_type == FileType.TEXT:
                ...
                return self._parse_audio(file_bytes, filename, mime_type, file_size)
            elif file_type == FileType.VIDEO:
                return self._parse_video(file_bytes, filename, mime_type, file_size)
    ...
    def _detect_encoding(self, data: bytes) -> tuple[str, float]:
    ...
    def _parse_text(
        ...
        """Parse text/code files."""
        encoding, confidence = self._detect_encoding(data)
        ...
        warnings = self._check_dangerous_patterns(content)
        ...
        if truncated:
            warnings.append(
                f"Content truncated to {self._config.max_text_chars:,} characters"
            )
    ...
    def _parse_doc_legacy(
        ...
        """Parse legacy .doc files (limited support)."""
        return ParseResult(
            success=True,
    ...
    def _parse_rtf(
        ...
        try:
            ...
            text = re.sub(r"[{}]", "", text)
            content = clean_extracted_text(text)
        except Exception as e:
    ...
    def _parse_csv(
        ...
        """Parse CSV files."""
        encoding, _ = self._detect_encoding(data)
    ...
    def _parse_xls_legacy(
        ...
        """Parse legacy .xls files (limited support)."""
        return ParseResult(
            success=True,
    ...
    def _parse_ppt_legacy(
        ...
        """Parse legacy .ppt files (limited support)."""
        return ParseResult(
            success=True,
    ...
    def _parse_image(
        ...
        """Parse image files (extract metadata, no text content)."""
    ...
    def _parse_audio(
        ...
        """Parse audio files (metadata only, no transcription)."""
        metadata: Dict[str, Any] = {
            "format": Path(filename).suffix.lower().lstrip("."),
            "is_audio": True,
        }
        ...
        return ParseResult(
            success=True,
            content="",  # Audio files - transcription not implemented
            file_type=FileType.AUDIO,
            filename=filename,
            mime_type=mime_type,
            file_size=file_size,
            metadata=metadata,
            warnings=["Audio transcription not available"],
        )
    ...
    def _parse_video(
        ...
        """Parse video files (metadata only)."""
        metadata: Dict[str, Any] = {
            "format": Path(filename).suffix.lower().lstrip("."),
            "is_video": True,
        }
        ...
        return ParseResult(
            success=True,
            content="",  # Video files - no text content
            file_type=FileType.VIDEO,
            filename=filename,
            mime_type=mime_type,
            file_size=file_size,
            metadata=metadata,
            warnings=["Video content not processed"],
        )
    ...
    def _parse_binary(
        ...
        """Parse binary files (no text extraction)."""
        return ParseResult(
            success=True,
