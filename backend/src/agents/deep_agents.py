...
# MessageValidationMiddleware to fix null tool_call IDs before LLM calls
from backend.src.agents.middleware.message_validation_middleware import MessageValidationMiddleware
...
def build_deep_middleware(
    ...
    *,
    ...
    # 0. MessageValidationMiddleware - Fix null tool_call IDs before LLM calls
    # This MUST be first to intercept all model calls and ensure valid messages
    middleware.append(MessageValidationMiddleware())
    logger.debug("Added MessageValidationMiddleware for tool_call ID validation")
    ...
    # 
    # Architecture:
    # - BackgroundSubagentMiddleware: Enables async/parallel subagent execution (opt-in)
    # - SubAgentMiddleware: Allows spawning subagents (independent of background tasks)
    # - ToolCallCounterMiddleware: Tracks tool usage per subagent (only with background tasks)
    #
    # The two middleware are INDEPENDENT - you can have subagents without background tasks.
    
    # 3.1 Background Task Middleware (opt-in)
    # Creates registry for tracking background tasks and tool call counts
    counter_middleware = None
    ...
    if enable_background_tasks:
        ...
        # Create counter middleware for subagent injection
        # This enables 'task_progress' to show tool call counts
        counter_middleware = ToolCallCounterMiddleware(bg_middleware_instance.registry)
        logger.debug("Created ToolCallCounterMiddleware for subagent injection")

    # 3.2 SubAgent Middleware - Spawn sub-agents for parallel work
    # This is INDEPENDENT of background tasks - subagents work either way
    if SUBAGENT_AVAILABLE and SubAgentMiddleware is not None and subagents:
        # If background tasks enabled, inject counter middleware into each subagent
        if counter_middleware:
            for subagent in subagents:
                # Handle both SubAgent objects and dict representations
                if hasattr(subagent, 'middleware'):
                    # SubAgent object
                    existing_mw = list(subagent.middleware) if subagent.middleware else []
                    subagent.middleware = [counter_middleware] + existing_mw
                elif isinstance(subagent, dict) and 'middleware' in subagent:
                    # Dict representation
                    existing_mw = list(subagent['middleware']) if subagent['middleware'] else []
                    subagent['middleware'] = [counter_middleware] + existing_mw
                # If neither, skip injection for this subagent
            logger.debug(f"Injected ToolCallCounterMiddleware into {len(subagents)} subagent(s)")
        
        middleware.append(
            SubAgentMiddleware(
                default_model=model,
                default_tools=apply_tool_filter("general_subagent", tools) if tools else [],
                subagents=subagents,
                default_middleware=[
                    TodoListMiddleware(),
        ...
        logger.debug(f"Added SubAgentMiddleware with {len(subagents)} subagent(s)")
    elif not subagents:
        logger.debug("No subagents provided - SubAgentMiddleware not added")
    else:
...
def create_agent(
    ...
    *,
    # Direct system prompt override (optional, takes precedence over prompt_template)
    ...
    Args:
        ...
        tools: Tools available to the agent.
        prompt_template: Name of the prompt template to load from backend.src.prompts.
        system_prompt: Optional direct system prompt string. If provided, takes
            precedence over prompt_template (useful for modules with local prompts).
    ...
    # Get the system prompt - system_prompt parameter takes precedence over prompt_template
    loaded_prompt = None
    if system_prompt:
        # Direct system prompt provided - use it directly
        loaded_prompt = system_prompt
        logger.debug(f"Using provided system_prompt for agent '{agent_name}'")
    elif prompt_template:
        # Load from template file (backend.src.prompts)
        try:
            loaded_prompt = get_prompt_template(prompt_template)
            ...
            loaded_prompt = f"You are a helpful {agent_type} agent named {agent_name}."
    
    # Combine with base agent prompt for deep agent features
    if loaded_prompt:
        full_prompt = f"{loaded_prompt}\n\n{BASE_AGENT_PROMPT}"
    else:
