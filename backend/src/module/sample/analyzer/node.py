...
from backend.src.module.sample.template import apply_prompt_template, get_prompt_template
...
# ADJUST THIS 
from backend.src.module.sample.types import State
from backend.src.config.tool_filter_config import apply_tool_filter
from .subagents import get_subagents

# Import CompiledSubAgent for workflow integration
from backend.src.agents.subagents import CompiledSubAgent, SUBAGENT_AVAILABLE
        ...
        async for chunk in agent.astream(
            ...
            if stream_mode == "messages":
                if isinstance(data, tuple) and len(data) == 2:
                    ...
                    if hasattr(message, "content") and isinstance(collected_messages, list):
...
async def _setup_and_execute_agent_step(
    ...
    *,
    ...
    if SUBAGENT_AVAILABLE and CompiledSubAgent is not None:
        try:
            from backend.src.module.sample.sub_builder import graph as workflow_graph
