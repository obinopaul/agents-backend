...
from backend.src.module.utils import append_to_system_message
...
def _get_subagents(
    *,
    ...
    if general_purpose_agent:
        ...
        # Apply tool filter for general-purpose subagent
        filtered_default_tools = apply_tool_filter("general_subagent", default_tools)
        general_purpose_subagent = create_agent(
            default_model,
            system_prompt=DEFAULT_SUBAGENT_PROMPT,
            tools=filtered_default_tools,
    ...
    for agent_ in subagents:
        ...
        # Apply tool filter using subagent role (falls back to name)
        subagent_role = agent_.get("role", agent_["name"])
        _tools = apply_tool_filter(subagent_role, _tools)
...
def _create_task_tool(
    *,
    ...
    def task(
        ...
        # FIX: Pass config to subagent so CompiledSubAgent receives mcp_url, thread_id, etc.
...
# Flag to indicate this local fork is available
